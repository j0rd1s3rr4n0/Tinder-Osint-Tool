...
if (typeof first_user_passed === 'undefined') {var first_user_passed = false;}else {first_user_passed = true;}
...
function findUser(){
	if(find && (user_to_search!="")){
		...
		if(document.querySelector("div.recsCardboard__cardsContainer.H\\(100\\%\\).Pos\\(r\\).Z\\(1\\) > div > div.Pos\\(a\\).B\\(0\\).Iso\\(i\\).W\\(100\\%\\).Start\\(0\\).End\\(0\\) > div > div.Mx\\(a\\).Fxs\\(0\\).Sq\\(70px\\).Sq\\(60px\\)--s.Bd.Bdrs\\(50\\%\\).Bdc\\(\\$c-ds-border-gamepad-nope-default\\) > button")){
			var pers = document.querySelectorAll(".focus-button-style > div > div > div > div > div > div > div > span");
            // Crear un objeto con las claves deseadas
            var customKeys = {};
            ...
            var name = customKeys.actual.innerText;
